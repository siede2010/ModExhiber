...
void main() {
    ...
    vec4 maxed = max(max(texture2D(u_texture, v_texCoords.xy + vec2(0.0, step) * v),texture2D(u_texture, v_texCoords.xy + vec2(0.0, -step) * v)),max(texture2D(u_texture, v_texCoords.xy + vec2(step, 0.0) * v),texture2D(u_texture, v_texCoords.xy + vec2(-step,0.0) * v)));
